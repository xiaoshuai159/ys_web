      ...
      <div class="shadow-div">
        <div class="base_table">
          <el-table v-loading="tableDataLoading" element-loading-text="数据加载中" element-loading-spinner="el-icon-loading"
            ...
            <el-table-column prop="task_name" label="任务名称" :show-overflow-tooltip="true" min-width="150">
            </el-table-column>
            <!-- <el-table-column prop="ip_num" label="IP数量" :show-overflow-tooltip="true" width="90">
            </el-table-column> -->
            <el-table-column prop="ip_num" label="IP/域名数量" :show-overflow-tooltip="true" min-width="120">
            </el-table-column>
            <el-table-column prop="created" label="创建时间" :show-overflow-tooltip="true" min-width="190">
              <template slot-scope="{ row }">
                {{ formatDate(row.created) }}
              </template>
            </el-table-column>
            <el-table-column prop="status" label="任务状态" :show-overflow-tooltip="true" min-width="120">
            ...
            <el-table-column label="操作" min-width="220" fixed="right">
              <template slot-scope="scope">
                ...
                <el-button class="operator-table-btn red" type="warning" size="mini" v-if="scope.row.status == '等待下发'"
                  @click="qxTask(scope.row.id)">取消</el-button>
                ...
                <el-button class="operator-table-btn green" type="warning" size="mini" @click="putTask(scope.row)"
                  v-if="scope.row.status == '已完成'">导出</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
            :current-page="currentPage" :page-size="pageSize" layout="total, prev, pager, next" :total="taskTotal"
      ...
      <div>
        <el-form ref="dialogForm" :inline="true" :model="dialogForm" :rules="rules">
          ...
          <el-form-item prop="taskType" label="任务类型：">
            <el-select v-model="dialogForm.taskType">
          ...
          <el-form-item label="上传文件：" prop="file">
            <el-upload class="upload-demo" action="#" :on-preview="handlePreview" :auto-upload="false"
              :on-change="handleFileChange" :file-list="dialogForm.fileList" :http-request="httpRequest">
              <el-button size="small" type="primary" style="width:100px">点击上传</el-button>
              <div slot="tip" class="el-upload__tip">只能上传csv文件，且不超过5MB</div>
            </el-upload>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    putTask(row) {
      ...
      formData.append('file_type', row.file_type)
      // const { data: res } = await this.$http.get(`/tasks/${rowId}/ips`,{params: {
      this.$http.get(`/tasks/${row.id}/export`, { responseType: 'blob' }) // 指定响应类型为 blob
        .then(response => {
    ...
    handleFileChange(file, fileList) {
      ...
      // 将文件列表保存到 dialogForm 中
      this.dialogForm.fileList = fileList;
    },
    httpRequest(params) {
      console.log(params);
      this.dialogForm.fileList = params.file;
      // if (this.dialogForm.taskName.leng
    },
    cjClick() {
      // const fileList = this.$refs.upload.uploadFiles;

      // if (this.dialogForm.taskName.length == 0 || this.dialogForm.IP.length == 0) {
      if (this.dialogForm.taskName.length == 0) {
        this.$message('请输入完整')
        return false
      }
      const formData = new FormData()
      formData.append('task_name', this.dialogForm.taskName)
      formData.append('task_type', this.dialogForm.taskType)
      formData.append('file', this.dialogForm.fileList[0].raw)
      // this.$http.post("/tasks", {
      //   task_name: this.dialogForm.taskName,
      //   ips: this.dialogForm.IP
      // }).
      this.$http.post("/tasks/", formData, {
        headers: {
        ...
        if (res.code == 200 || res.length != 0) {
          ...
          this.tableData = res.data
          // this.tableData.pop()
          // this.tableData.unshift({task_name:'task99',ip_num:100,create_time:dayjs().format('YYYY-MM-DD HH:mm:ss'),status:"等待下发"})
          // this.taskTotal++
          this.tasklist()
        } else {
          this.$message({
    ...
    async goResult(row) {
      ...
      localStorage.setItem("curRowTaskType", JSON.stringify(row.task_type))
      console.log(status);
